...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions: 
            position = distribution.argMax() # get the key with the greatest value 
        ...
        shortest_distance = 999999 
        closest_ghost_action = None 
        # figure out which action minimizes distance to closest ghost 
        for action in legal: 
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successorPosition, closest_ghost_position)
            if distance < shortest_distance: 
                shortest_distance = distance 
                closest_ghost_action = action

        return closest_ghost_action 
